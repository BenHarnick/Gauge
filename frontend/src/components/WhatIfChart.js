...
export function WhatIfChart({ data, loading, error, feature }) {
    ...
    if (loading) {
        return (_jsx("div", { className: "rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-500", children: "Running what-if sweep..." }));
    }
    if (error) {
        return (_jsx("div", { className: "rounded-lg border border-red-200 bg-red-50 p-6 text-sm text-red-700", children: error }));
    }
    if (rows.length === 0) {
        return (_jsx("div", { className: "rounded-xl border border-dashed border-slate-200 bg-white p-6 text-sm text-slate-500", children: "Pick a feature to sweep and the curve will render here." }));
    }
    return (_jsxs("div", { className: "rounded-xl border border-slate-200 bg-white p-4 shadow-card", children: [_jsxs("div", { className: "mb-2 text-sm text-slate-600", children: ["Charges and member out-of-pocket as ", _jsx("strong", { children: feature }), " varies. Solid lines are point estimates; dashed lines are the 10th and 90th percentile bounds."] }), _jsx("div", { className: "h-80 w-full", children: _jsx(ResponsiveContainer, { width: "100%", height: "100%", children: _jsxs(LineChart, { data: rows, margin: { top: 10, right: 16, bottom: 0, left: 0 }, children: [_jsx(CartesianGrid, { stroke: "#e2e8f0", strokeDasharray: "3 3" }), _jsx(XAxis, { dataKey: "value", tick: { fontSize: 12, fill: "#475569" }, padding: { left: 8, right: 8 } }), _jsx(YAxis, { tick: { fontSize: 12, fill: "#475569" }, tickFormatter: (v) => v >= 1000 ? `$${Math.round(v / 1000)}k` : `$${v}`, width: 56 }), _jsx(Tooltip, { formatter: (v, name) => [
