...
from health_app.predictor.dataset import FEATURE_COLUMNS, TARGET_COLUMN
...
def _pick_column(df: pd.DataFrame, role: str) -> str:
    ...
    raise ValueError(
        ...
        f"COLUMN_CANDIDATES[{role!r}] in src/health_app/predictor/meps.py."
    )
    ...
    if saq_path is not None:
        ...
        if not bmi_cols_in_saq:
            raise ValueError(
                ...
                f"in src/health_app/predictor/meps.py. Run "
